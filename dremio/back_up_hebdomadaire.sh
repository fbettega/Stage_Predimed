...
LANG=C DOW=$(( 1 + $(date +%V) - $(date -d "$(date -d "-$(($(date +%d)-1)) days")" +%V) )) #recuperation du jour de la semaine
...
ssh ${LOGNAME}@proback89 "rm -r /opt/dremio_backup_auto/hebdomadaire/${DOW}"

eval "scp -r ./dremio_backup_auto/${DOW} ${LOGNAME}@proback89:/opt/dremio_backup_auto/hebdomadaire"
