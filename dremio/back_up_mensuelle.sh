...
LANG=C DOW=$(date -d "$D" '+%m') #recuperation du jour de la mois
...
ssh ${LOGNAME}@proback89 "rm -r /opt/dremio_backup_auto/mensuelle/${DOW}"

eval "scp -r ./dremio_backup_auto/${DOW} ${LOGNAME}@xxyy:/opt/dremio_backup_auto/mensuelle"
